...
async function loadHistForLocation(lat,lng){
  try{
    ...
    window.histLayer=L.tileLayer(`https://geoservices.hisgis.nl/tiles/minuutplans/{z}/{x}/{y}.png?cut${code}*`,{opacity:Number(opSlider.value)/100||0.6,maxZoom:19}).addTo(map);
...
map.on("zoomend",()=>{
  // Op zoom 20 blijft OSM zichtbaar; de 1832-overlay wordt daar verborgen.
  if(map.getZoom()>=20 && window.histLayer && map.hasLayer(window.histLayer)){
    map.removeLayer(window.histLayer);
  }else if(map.getZoom()<20 && window.histLayer && !map.hasLayer(window.histLayer)){
