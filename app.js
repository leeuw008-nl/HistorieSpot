...
// ========================================
// KLIK OP HISTORISCH MINUUTPLAN
// RCE GetFeatureInfo
// ========================================

map.on("click", async function (e) {

    // Alleen reageren als de minuutplanlaag zichtbaar is
    ...
    const params = new URLSearchParams({
        ...
        layers: "Minuutplanbegrenzingen",
        query_layers: "Minuutplanbegrenzingen",
        ...
        bbox: [
            map.project(bounds.getSouthWest(), map.getZoom()).x,
            map.project(bounds.getSouthWest(), map.getZoom()).y,
            map.project(bounds.getNorthEast(), map.getZoom()).x,
            map.project(bounds.getNorthEast(), map.getZoom()).y
        ].join(",")
    });
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error("RCE-server gaf HTTP " + response.status);
        }
        ...
        let inhoud = `
            <strong>Kadastraal minuutplan</strong><br>
            <small>RCE · 1811–1832</small>
            <hr>
        `;
        ...
        console.error("Fout bij ophalen historische gegevens:", error);
